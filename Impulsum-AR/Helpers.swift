...
//  Impulsum-AR
//
//  Created by Dason Tiovino on 16/10/24.
//
...
func isConvex(_ prev: SIMD3<Float>, _ curr: SIMD3<Float>, _ next: SIMD3<Float>, area: Float) -> Bool {
...
func generateMesh(_ points: [SIMD3<Float>])->[UInt32]{
    ...
    if area < 0 {
        // Reverse indices for clockwise orientation
    ...
    while remainingVertices.count > 3 {
        ...
        for i in 0..<n {
            ...
            if isConvex(prevPoint, currPoint, nextPoint, area: area) {
                var ear = true
                for j in 0..<n {
                    ...
                    if isPointInTriangle(SIMD2<Float>(pPoint.x, pPoint.z),
